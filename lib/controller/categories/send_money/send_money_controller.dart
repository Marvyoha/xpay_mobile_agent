...
class SendMoneyController extends GetxController {
  ...
  void onInit() {
    ...
    copyInputController.addListener(() {
      moneyControllerText.value = copyInputController.text;
      if (_debounce?.isActive ?? false) _debounce?.cancel();
      _debounce = Timer(const Duration(milliseconds: 500), () {
        if (_isValidEmail(copyInputController.text)) {
          getCheckUserExistDate();
        }
      });
      if (!_isValidEmail(copyInputController.text) &&
  ...
  updateExchangeRate() {
    receiverExchangeRate.value =
        double.parse(selectReceiverWallet.value!.currency.rate.toString()) /
